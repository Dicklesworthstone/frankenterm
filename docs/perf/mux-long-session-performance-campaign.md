...
| Same-grid invalidation | Duplicate resize invalidation consolidated in source; coarse same-grid invalidation remains | Retain native causal A/B evidence for the consolidation (`4tenz.8.3`); then separately test splitting surface/projection from grid damage | Roll back on SSIM/cursor/selection/IME/a11y regression or no tail win |
