...
| ------- | ---------- | -------- | ------ |
| 60      | 2026-08-17 | additive | adds PDU96/97 `ReliableKeyEventV1` request/typed response for one ordered key-down/key-up lane. The request carries the existing nonzero client input serial; the response distinguishes applied, duplicate-applied, bounded scheduler retry, duplicate-pending, and terminal refusal outcomes without free-form text. Both schemas have exact bounded decoders, peers must gate them on the agreed v60 dialect, and v58/v59 peers retain the legacy input path. See ft-interactive-systems-performance-4tenz.2.4.8.2. |
