...
| ------- | ---------- | -------- | ------ |
| 64      | 2026-08-24 | additive | adds PDU100/101 `ReliablePaneWriteV1` request/typed response for bounded arbitrary pane bytes on the same ordered client ledger as reliable key transitions. The server fingerprints the exact pane registration, input serial, payload length, and SHA-256 payload digest before one `Write::write` call; replies retain an exact applied prefix for ACK-loss replay, distinguish proven-zero retry/rejection, and quarantine panics or ambiguous outcomes. The client owns at most a bounded FIFO prefix, counts in-flight work against both event and byte caps, keeps a partially applied suffix ahead of later keys/writes under a pre-reserved fresh serial, and refuses pre-v64 peers without a legacy write shim. The retained ledger is mux-process-local, so crash-durable replay remains explicitly unclaimed pending the guardian input WAL. The minimum supported dialect remains 61. See ft-6vfeq.2. |
| 63      | 2026-08-20 | additive | adds PDU99 `SendPasteTracedV1`, a bounded sampled wrapper around the unchanged PDU13 `SendPaste` request. Only peers that negotiated v63 emit or accept the traced form; older supported peers retain byte-identical PDU13 paste traffic. The wrapper carries the content-free paste trace authority needed to continue a sampled paste from client K3 to server K4/K5 while the operational paste bytes remain solely in the request and never enter trace events. The minimum supported dialect remains 61. See ft-interactive-systems-performance-4tenz.2.4.1.1. |
| 62      | 2026-08-19 | additive | adds PDU98 `ReliableKeyEventTracedV1`, a fixed-shape sampled wrapper around the unchanged PDU96 `ReliableKeyEventV1` request. Only the first effect-eligible wire attempt may carry the content-free keypress trace context; authority probes, typed scheduler retries, duplicate-pending replies, ambiguous transport retries, and all later attempts remain on PDU96 so K4/K5 cannot be duplicated and reliable pane/serial deduplication cannot be bypassed. PDU95 remains a historical codec assignment but is not an active client or server endpoint. PDU98 has an exact 64 KiB decompressed/128 KiB compressed-body bound, is gated on the agreed v62 dialect, and v61 peers retain byte-identical PDU96. The minimum supported dialect remains 61. See ft-interactive-systems-performance-4tenz.2.4.1. |
| 61      | 2026-08-19 | breaking | replaces existing positional PDU0 `ErrorResponse` free-form text with a fixed-shape, content-free authority envelope: schema version, exact request PDU identity, finite rejection code, optional typed mux object identity, mutation-effect certainty, and retry/reconciliation authority. Unknown future numeric authorities remain frame-decodable but fail closed; local producers reject unknown or contradictory combinations before encoding. CLI fallback projects the same validated authority through the bounded `FRANKENTERM_MUX_ERROR_V1` record, including expected and observed request identities; malformed, contradictory, mismatched, or future records become `unknown_future` with no retry authority. The minimum supported dialect advances atomically to 61 so no v60 peer can misdecode the new PDU0 body. See ft-interactive-systems-performance-4tenz.5.5.14.7. |
| 60      | 2026-08-17 | additive | adds PDU96/97 `ReliableKeyEventV1` request/typed response for one ordered key-down/key-up lane. The request carries the existing nonzero client input serial and a server-issued exact pane-registration identity, preventing a delayed key from being redirected to a same-ID successor; an initial request without that identity is a no-effect authority probe. The response distinguishes authority binding, applied, duplicate-applied, bounded scheduler retry, duplicate-pending, and terminal refusal outcomes without free-form text. Both schemas have exact bounded decoders, peers must gate them on the agreed v60 dialect, and v58/v59 peers retain the legacy input path. See ft-interactive-systems-performance-4tenz.2.4.8.2. |
| 59      | 2026-08-17 | additive | adds sampled-only PDU95 `SendKeyDownTracedV1`, carrying the fixed, content-free `SampledTraceContextV1` beside the operational key request under a 64 KiB decompressed body ceiling. Unsampled input retains the byte-identical PDU11 schema; clients must gate PDU95 on the agreed v59 dialect, and v58 peers remain supported. See ft-interactive-systems-performance-4tenz.2.4.1. |
| 58      | 2026-08-14 | breaking | extends existing positional PDU4 `ListPanesResponse`, PDU82 `ListPanesCoherentResponse`, and PDU87 `ListPanesOrderedV1Response` with bounded complete floating-pane state, and adds the exact revision-stamped floating-spawn topology transition needed to publish registration, ownership, geometry, and focus as one committed result. Older payloads end before the required field and cannot reconstruct floating owners, so the minimum supported dialect advances atomically to 58 and all GUI/client/mux-server components must be redeployed together. See ft-interactive-swarm-product-convergence-7xqz4.8.10.3.2.2. |
| 57      | 2026-08-09 | additive | adds bounded request/response PDUs 93/94 for request-ordered, lightweight tiered-scrollback health across at most 256 unique panes. The response carries only ten content-free health counters plus typed unavailable/missing/closed/callback-panic outcomes; negotiated v56 peers remain supported and take the bounded legacy fallback without writing PDU93. The minimum supported dialect remains 56. See ft-interactive-systems-performance-4tenz.29 and source commits `e5da8d2e4`, `829460563`, `1f455c2e0`, `e98daa9dc`, and `80b7928f3`. |
