...
| `runtime_async` cancel semantics | Cancelled waits across runtime primitives terminate through the `Cx` path without leaking permits, joins, or messages. | `runtime-async-cancel.tla` | `crates/frankenterm-core/src/runtime_async.rs` | covered | `ft-tf6g3.18.2` |
