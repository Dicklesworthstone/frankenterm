...
| `Window::{push, insert, remove_by_idx, remove_tab_if_same, save_and_then_set_active}` | Live window state now carries a non-wrapping `WindowOrderRevision`, and `Window::order_snapshot` freezes exact ordered `Arc<Tab>` identities plus active identity. Individual mutators advance that live revision, but the low-level methods alone do not bind every mutation to one incarnation-scoped topology transaction or idempotent protocol result. | The revisioned live substrate is present. Complete server authority still requires every order-affecting path to publish one coherent topology transition; mixed-overlay intent must not be mistaken for a server commit. |
| `Mux::{add_tab_to_window, remove_tab, remove_tab_local_only, move_tab_between_windows}` | These preserve exact live `Arc<Tab>` identities. Same- and cross-window moves now preflight membership, capacity, and non-wrapping order revisions before mutation, but the ordinary production client/server path still emits and consumes legacy topology effects rather than the dormant ordered-window stream. | Pure-window membership/order changes belong to server authority. Finish all-mutator frozen topology publication and activate the negotiated ordered protocol before claiming reconnect durability; local mirror cleanup and mixed composition cannot rewrite remote order. |
