...
Each `ft robot <command>` dispatch returns either a named typed struct in
`frankenterm_core::robot_types`, a native-dispatch struct in
...
|---------|------|-------------|
| `robot state` | `StateWithTextData` | Pane metadata; `--include-text` attaches tail text per pane |
| `robot state` (array mode) | `Vec<PaneStateData>` | Bare per-pane rows emitted when `--include-text` is off |
| `robot get-text` | `GetTextData` | Pane text with truncation info |
| `robot get-text --panes / --all` | `BatchGetTextData` | Batched tail reads keyed by pane_id |
...
| Command | Type | Description |
|---------|------|-------------|
| `robot search` | `SearchData` | Lexical/semantic/hybrid results with mode-aware scores |
| `robot search-explain` | `SearchExplainData` | Plan + lint + mode selection trace for a query |
| `robot search-index stats` | `SearchIndexStatsData` | Per-index row counts and size snapshots |
| `robot search-index status` | `SearchPipelineStatusData` | Pipeline-level operational state (pause, rebuild, tier health) |
| `robot search-index metrics` | `SearchMetricsData` | Counter/latency snapshot for the search pipeline |
| `robot search-index reindex` | `SearchIndexReindexData` | Reindex job submission result |
| `robot evaluate-anomaly` | `serde_json::Value` (untyped) | Semantic-anomaly detector dump; no typed struct today |
...
| `robot workflow status --all` | `WorkflowStatusListData` | All known executions, one row per run |
| `robot workflow status <id> --detail` | `WorkflowStatusDetailData` | Step-by-step log for a single execution |
| `robot workflow abort` | `WorkflowAbortData` | Abort confirmation |

### Rules & pattern tooling
...
| Command | Type | Description |
|---------|------|-------------|
| `robot checkpoint save` | `CheckpointSaveData` | Snapshot/session checkpoint creation result |
| `robot checkpoint list` | `CheckpointListData` | Available checkpoints with pagination metadata |
| `robot checkpoint show` | `CheckpointShowData` | Checkpoint detail including pane snapshots |
| `robot checkpoint delete` | `CheckpointDeleteData` | Deletion receipt with freed-byte count |
| `robot checkpoint rollback` | `CheckpointRollbackData` | Rollback or dry-run rollback receipt |

### Context

| Command | Type | Description |
|---------|------|-------------|
| `robot context status` | `ContextStatusData` | Per-pane context budgets plus fleet pressure summary |
| `robot context rotate` | `ContextRotateData` | Durable context-rotation receipt |
| `robot context history` | `ContextHistoryData` | Per-pane compaction/rotation history |

### Work

| Command | Type | Description |
|---------|------|-------------|
| `robot work claim` | `WorkClaimData` | Work-claim receipt for one item and agent |
| `robot work release` | `WorkReleaseData` | Release receipt and resulting status |
| `robot work complete` | `WorkCompleteData` | Completion receipt with newly unblocked items |
| `robot work list` | `WorkListData` | Work queue rows matching the filter |
| `robot work ready` | `WorkReadyData` | Ready work rows plus ready/blocked counts |
| `robot work assign` | `WorkAssignData` | Assignment receipt and strategy used |

### Fleet

| Command | Type | Description |
|---------|------|-------------|
| `robot fleet status` | `FleetStatusData` | Fleet counts by agent state/program plus work queue summary |
| `robot fleet scale` | `FleetScaleData` | Scale dry-run or mutation receipt |
| `robot fleet rebalance` | `FleetRebalanceData` | Rebalance dry-run or mutation receipt |
| `robot fleet agents` | `FleetAgentsData` | Agent slot rows with optional filters |

### Profile

| Command | Type | Description |
|---------|------|-------------|
| `robot profile list` | `ProfileListData` | Available profile summaries |
| `robot profile show` | `ProfileShowData` | Full profile definition |
| `robot profile apply` | `ProfileApplyData` | Profile dry-run or apply receipt |
| `robot profile validate` | `ProfileValidateData` | Profile validation verdict and issues |

### Missions

| Command | Type | Description |
|---------|------|-------------|
| `robot mission state` | `MissionStateData` | Mission lifecycle snapshot (assignments, gates, cancel/retry counters) |
| `robot mission assign/unassign` | `MissionAssignmentData` | Assignment mutation result |
| `robot mission decide` | `MissionDecisionData` | Single decision-point write result |
| `robot mission decisions` | `MissionDecisionsData` | Full decision log for a mission |

### Transactions

| Command | Type | Description |
|---------|------|-------------|
| `robot tx plan` | `TxPlanData` | Plan structure: steps, preconditions, risk, compensations |
| `robot tx run` | `TxRunData` | Execution-start envelope with per-step records |
| `robot tx resume` | `TxResumeData` | Resume-from-checkpoint envelope |
| `robot tx rollback` | `TxRollbackData` | Rollback outcome with executed compensating actions |
| `robot tx show` | `TxShowData` | Timeline + chain-verification view of a prior run |

### Agents

| Command | Type | Description |
|---------|------|-------------|
| `robot agents list / show` | `AgentInventoryData` | Registered agent inventory |
| `robot agents configure` | `AgentConfigureData` | Configuration write result |
| `robot agents configure --dry-run` | `AgentConfigureDryRunData` | Preview of a configure call without mutation |
...
### Health & coding-agent session store (cass)
...
All types in `frankenterm_core::robot_types` and
`frankenterm_core::robot_ntm_surface` derive `Serialize` + `Deserialize`
and use `#[serde(default)]` for optional fields where older ft versions may
omit data, so typed clients can tolerate additive response changes. The
`robot_ntm_surface` module name is historical: it now houses the native
checkpoint/context/work/fleet/profile request and response contracts plus the
NTM-differential proof surface. The cass types (`CassSearchResult`,
`CassViewResult`, `CassStatus`) live in `frankenterm_core::cass` for the same
reason the cass CLI client lives there — they track the cass daemon's wire
contract, not the robot envelope. `EvaluateAnomaly` is intentionally untyped
today; treat its `data` field as a `serde_json::Value` and use
`parse_response_untyped` for it.
