...
| 3 | Verification covers correctness + basic performance non-regression | **30 HTTP client contract tests + 12 TLS contract tests + 2 service-boundary cx contract tests + 2 primitive contract tests** (§2.1, §2.2, §2.3, §2.6) |
...
output. Final summary line: `ft-xbnl0.2.4 — all 6 runs PASS (84 tests)`.
Exit 0 iff all 84 tests pass.
...
- [x] **Local smoke**: `./scripts/check_ft_xbnl0_2_4.sh` exits 0 with all 84 tests passing (30 HTTP + 45 TLS + 3 guards + 3 metrics + 1 web + 2 primitive). Confirmed at every post-tick run.
