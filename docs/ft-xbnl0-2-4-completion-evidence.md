...
| 3 | Verification covers correctness + basic performance non-regression | **24 HTTP client contract tests + 12 TLS contract tests + 2 service-boundary cx contract tests** (§2.1, §2.2, §2.3) |
...
- [ ] `rch exec -- ./scripts/check_ft_xbnl0_2_4.sh` exits 0 (all 76 tests pass: 24 HTTP + 45 TLS + 3 guards + 3 metrics + 1 web)
