...
| 3 | Verification covers correctness + basic performance non-regression | **29 HTTP client contract tests + 12 TLS contract tests + 2 service-boundary cx contract tests + 2 primitive contract tests** (§2.1, §2.2, §2.3, §2.6) |
...
output. Final summary line: `ft-xbnl0.2.4 — all 6 runs PASS (83 tests)`.
Exit 0 iff all 83 tests pass.
