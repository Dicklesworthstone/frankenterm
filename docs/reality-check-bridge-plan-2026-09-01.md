...
**Design.** Add `storage::policy_state` (get/set through the generic config-KV surface, no new table if `config_kv` suffices — it does: key `policy.kill_switch`, JSON value); the engine writes on every tier change and reads at construction; doctor drops the process-local caveat when the persisted row exists and shows its age.
...
Not done today: signed release (needs an operator-driven `dsr build/release` with signing on), same-generation app for the native attach e2e, live-loop tier 1 with agents, `ft setup shell-integration`, kill-switch persistence, the capture-counter realignment.
