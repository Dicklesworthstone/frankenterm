...
**G74 part 2 (2026-09-02 10:20–10:40 UTC, ft-xxfwy.32, commit 29af4e3f6).** The permanent "Sequence discontinuity" after one dropped segment was a pipelining effect, not a missing resync: the `PaneCursor` is shared between the capture producers and the persistence loop, so by the time the loop sees a mismatch the producers have already numbered and queued later segments; `resync_seq` reset `next_seq` to storage's seq + 1 regardless, the next capture reused a number still in flight, and the mismatch reappeared on every segment. `PaneCursor::realign_next_seq` now applies the storage-vs-producer offset once and lets the queued backlog drain with the known offset (debug, not warn). Three ingest tests cover the sequential, pipelined and storage-ahead cases; the headless observe smoke on the rebuilt dev binaries passed 7/7 with dropped segments 0 and sequence resyncs 0. Part 1 (the drop itself) is the `BEGIN IMMEDIATE` change from earlier in the day. Release hygiene from the same commit: two `unused_mut` bindings in the web_framework tests would have failed the DSR clippy gate and were removed before `dsr quality` ran on the release tree.

**Release gate findings (2026-09-02 16:00–23:30 UTC).** The first complete `cargo test --workspace` run of the DSR quality lane (from a clean clone of the release commit, because the shared checkout carried a sibling's uncommitted dependency batch mid-run) surfaced ten red CLI contract tests that pre-date this reality check; the tx ones and the exit-code one reproduce at the v0.15.1 tag. One was a real product defect and is fixed in 7d7181f0a: `ft session recover` refused every crash-orphaned scrollback file ("filename/header identity changed during recovery") because the orphan scan compares the raw on-disk header (a SIGKILLed writer never flushes `write_cursor_bytes`) against the header the record reader reconciles from the recovered tail; recovery now compares only the file's identity (format version, capacity, pane uuid, creation time). Three tests were corrected to the contracts actually in force (a valid empty topology snapshot for session checkpoints; "stderr has no panic and no duplicated error" instead of "stderr is empty", since ft has logged at info to stderr since before v0.15.1; exit 5 for `mission.tx.execution_failed`, the mapping since July). Six tx contract tests drive sends through the legacy WezTerm CLI stub, and pane input over the CLI has failed closed since da8e16eab (20 Aug); they are `#[ignore]`d with that reason and tracked by ft-ydqah (retarget to a mux-server harness). The workspace suite is therefore a truthful release gate again, with the six ignores disclosed here and in the CHANGELOG. Also in the release window: the DSR quality test timeout was raised to six hours for the pinned `-j 2` worker, the Windows build host's OpenSSH default shell was switched to cmd.exe for DSR (to be restored), and the release runs from `~/projects/ft-release-0152b` with DSR `local_path` redirected there (to be restored).
