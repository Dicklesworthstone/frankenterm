...
**Started:** 2026-06-14  |  **Updated:** 2026-08-19  |  **Project:** frankenterm  |  **Language:** Rust + GitHub Actions
...
- **Deferred:** Asupersync 0.4.8 and the FrankenSQLite backend until the
  single-runtime dependency cohort and storage-transaction prerequisites below
  are satisfied; the latest registry facade is `fsqlite 0.3.5`, while the
  latest FrankenSQLite project release is v0.3.6
...
- **Latest-stable boundary (re-audited 2026-08-19):** official crates.io and tagged-release evidence identify `asupersync 0.4.8` and `asupersync-macros 0.4.8` as latest stable. Adopting them now would make Cargo resolve incompatible 0.3.x and 0.4.x runtime/type universes because FrankenTerm's pinned FastMCP and FastAPI revisions still require 0.3.x and its pinned FrankenSearch revision explicitly requires `<0.4`. Stable FastMCP v0.6.0 has converged on exactly `asupersync 0.4.8`, but declares Rust 1.100 while FrankenTerm's current workspace MSRV is 1.95. The latest crates.io FastAPI is 0.3.1 and still requires `asupersync ^0.3.9`; FastAPI main retains that constraint. Stable FrankenSearch v1.6.0 requires `>=0.4.3,<0.5`. A single-runtime migration therefore still requires a stable 0.4-compatible FastAPI release plus an explicit workspace-MSRV decision (or a compatible FastMCP release); repinning only the ready consumers cannot solve the graph. Coordinated convergence remains tracked by `ft-wc3uc`; a split runtime graph or relaxed-constraint bypass is not acceptable.
- **Stable ecosystem gate:** stable FastMCP v0.6.0 requires exactly Asupersync 0.4.8, Rust 1.100, and `nightly-2026-08-19`. Its actual migration surface is bounded but semantic: `timeout_ms` becomes a validated `RequestTimeoutPolicy`; `Client::close` requires `&mut self` and returns `McpResult<()>`; the non-Cx `run_transport_returning` is gone in favor of `run_transport_returning_with_cx`; and `TestClient` moves behind the `testing-lab` feature. The server constructor, catalog accessors, `run_transport`, and `run_transport_with_cx` remain available, so no speculative builder rewrite is planned. FastAPI PR #29 demonstrates 0.4-family compatibility but is unmerged, unreleased, locked to 0.4.7, and still has a Clippy failure; it is evidence, not a stable gate. Stable FrankenSearch v1.6.0 already accepts `>=0.4.3,<0.5`, but its `hybrid` feature changes the lexical engine from Tantivy to Quill; an eventual repin must explicitly choose `lexical-tantivy` or qualify the backend change. Wait for a stable 0.4-compatible FastAPI release and resolve the FastMCP/workspace MSRV and search-backend decisions, then migrate the runtime pair and all three consumers as one reviewed cohort.
...
- **Cohort identities:** FastMCP v0.6.0 has checksum `b3504a65f892368640adf0b1b7f52e3f1a414e34b564f8eccfe138bbd5d74ddd`; annotated tag `f3436585ff9ee33bb9e1a3aa1a495719918c3003` resolves to `4c80d396eff0a2741b89ee88c0e26a36273ee14f`. FastAPI 0.3.1 has checksum `e1cf87d300a2f55de384b65dd69a9d066ffb8c2376e2b6e64f1c551763a7dacd` but no matching tag or GitHub Release; PR #29 head is `4b57b95ed5e34a7a8aea320e3a4ebfc74c12328c`. FrankenSearch v1.6.0 annotated tag `260d393ea7e83d5aee5774aaaa3d3e622ab0d727` resolves to `4302c377169c97d52296efa77fc37e66429aad44`; its facade remains versioned 0.3.2.
...
### FrankenSQLite v0.3.6 / registry fsqlite 0.3.5

- **Status:** researched, not yet integrated. FrankenTerm currently has no `fsqlite` dependency to update; the two FrankenSQLite features are empty default-off scaffolds. The recorder identity correction is present in the current tree: the real implementation is `RusqliteRecorderStorage`, reports `Rusqlite`, and FrankenSQLite selection fails closed instead of silently opening rusqlite. This is new backend architecture work, not a version bump.
- **Stable target (re-audited 2026-08-19):** crates.io `fsqlite 0.3.5` remains the latest published facade at checksum `0d5c359d988d336716ac1fe84f032a392cbd4b299ca17c2c01ab10c5d8179367`; lightweight tag `v0.3.5` resolves to `92a4e4e735483be136b8a73cc6bf3a5d6263dcf8`. The latest official FrankenSQLite project release is now v0.3.6; annotated tag `b793ce180a8c79089fd8b75e3c44105b71d3c52b` resolves to `56cf464d8b3681136c4b2e340c8fedbe1ae98c40`, but that release is not yet published as an `fsqlite 0.3.6` crate. Do not substitute a Git dependency for a missing stable facade release. FrankenTerm has no `fsqlite` package today, so this remains a new backend integration rather than an updater bump.
- **Runtime boundary:** both registry `fsqlite 0.3.5` and project release v0.3.6 require `asupersync >=0.4.3,<0.5`; v0.3.6's release lock uses the 0.4.8 line. The facade exposes runtime types publicly, so adding either source before the coordinated 0.4.x migration would create the forbidden split runtime universe.
- **Concurrency/API boundary:** the direct `fsqlite::Connection` is deliberately `!Send + !Sync`, while FrankenTerm's synchronous object-safe `StorageBackend` is `Send + Sync`. A reviewed `AsyncConnection` worker/actor adapter is the plausible canary, but it cannot make the current transaction guard exclusive: `ft-ig9lh` must first ensure one owner retains a transaction from begin through commit or rollback. `AsyncConnection` owns a dedicated thread and bounded mailbox per connection, currently with a 32-MiB worker stack, requires explicit asynchronous close, and takes `fsqlite_types::cx::Cx`; its native Asupersync context must be installed explicitly. The eventual design must use a small bounded pool rather than one connection per pane or agent. The landed recorder selection/identity correction is a regression guard, not evidence that this backend exists.
- **Supported-envelope boundary:** upstream documents verification for at most eight concurrent writers and does not support ten or more implicit-autocommit writers. The v0.3.6 release's eight-thread result does not justify a 128-writer or Threadripper-wide scaling claim. Default features include native, Linux io_uring, JSON, FTS5, RTree, ICU, and miscellaneous extensions, while `AsyncConnection` additionally needs `async-api`; the canary must use an explicit minimal reviewed feature set.
- **Sequencing:** finish the exact 0.3.10 proof; obtain a stable FastAPI release
  compatible with one 0.4.x runtime family; review it together with FastMCP
  v0.6.0 and the stable FrankenSearch target; move the
  Asupersync pair to 0.4.8 with one resolved family; repair transaction
