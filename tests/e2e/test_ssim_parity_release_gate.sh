...
grep -q 'ssim_oracle_corpus_retained_release_run_validated' crates/frankenterm-core/src/render_quality.rs
...
jq -e '
  ...
  and .[0].status == "retained_run_validated"
  and .[0].current_degradation == "none"
...
export RCH_MIRROR_REQUIRE_WORKSPACE_MEMBER_ROOTS="${RCH_MIRROR_REQUIRE_WORKSPACE_MEMBER_ROOTS:-1}"
export RCH_MIRROR_REQUIRE_ALL_CHECKED_WORKERS="${RCH_MIRROR_REQUIRE_ALL_CHECKED_WORKERS:-1}"
export RCH_MIRROR_REQUIRED_PATHS="${RCH_MIRROR_REQUIRED_PATHS:-Cargo.toml,crates/frankenterm-core-replay-types/src/lib.rs,crates/frankenterm-core/src/tui/mod.rs,crates/frankenterm-gui/tests/ssim_parity.rs}"
