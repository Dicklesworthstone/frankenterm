...
source "${GUARD_LIB}"
rch_init "${LOG_DIR}" "${RUN_ID}" "replay_ci_gates" "${REPO_ROOT}"
...
if run_rch_cargo_logged "${scenario1_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate::tests::gate1_all_smoke_pass && grep -q "ok" "${scenario1_log}"; then
...
if run_rch_cargo_logged "${scenario2_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate::tests::gate2_unit_test_failure && grep -q "ok" "${scenario2_log}"; then
...
if run_rch_cargo_logged "${scenario3a_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate::tests::gate3_all_pass && grep -q "ok" "${scenario3a_log}"; then
...
if run_rch_cargo_logged "${scenario3b_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate::tests::evidence_bundle_collects_artifact_paths && grep -q "ok" "${scenario3b_log}"; then
...
if run_rch_cargo_logged "${scenario4a_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate::tests::apply_waiver_changes_status && grep -q "ok" "${scenario4a_log}"; then
...
if run_rch_cargo_logged "${scenario4b_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate::tests::apply_expired_waiver_no_change && grep -q "ok" "${scenario4b_log}"; then
...
if run_rch_cargo_logged "${scenario5_log}" env CARGO_TARGET_DIR="${RCH_TARGET_DIR}" cargo test -p frankenterm-core --lib replay_ci_gate && grep -q "test result: ok" "${scenario5_log}"; then
