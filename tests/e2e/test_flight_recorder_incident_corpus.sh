...
DOCS_INDEX="docs/flight-recorder/README.md"
...
require_file "${OPERATOR_PLAYBOOK}"
...
rg -F -q "## 2D. Flight-Recorder Incident Replay Runbook" "${RUNBOOK}" || fail "operator runbook missing incident replay section"
rg -F -q "ft.swarm.incident_surfaces.v1" "${RUNBOOK}" || fail "runbook missing surface contract"
rg -F -q "fixtures/flight-recorder/incident-corpus/" "${RUNBOOK}" || fail "runbook missing corpus directory"
rg -F -q "fixtures/flight-recorder/incident-corpus/source-sets/remote-cargo-pass.json" "${RUNBOOK}" || fail "runbook missing remote pass source set"
rg -F -q "fixtures/flight-recorder/incident-corpus/source-sets/rch-mirror-before-cargo.json" "${RUNBOOK}" || fail "runbook missing RCH block source set"
rg -F -q "fixtures/flight-recorder/incident-corpus/source-sets/dirty-tree-contaminated.json" "${RUNBOOK}" || fail "runbook missing dirty-tree source set"
rg -F -q "fixtures/flight-recorder/incident-corpus/source-sets/agent-mail-outage-beads-fallback.json" "${RUNBOOK}" || fail "runbook missing Agent Mail outage source set"
rg -F -q "ft robot incidents list" "${RUNBOOK}" || fail "runbook missing list command"
rg -F -q "ft robot incidents show" "${RUNBOOK}" || fail "runbook missing show command"
rg -F -q "ft robot incidents explain" "${RUNBOOK}" || fail "runbook missing explain command"
rg -F -q "ft robot --format toon incidents replay" "${RUNBOOK}" || fail "runbook missing TOON replay command"
rg -F -q "wa://incidents/{incident_id}/explain" "${RUNBOOK}" || fail "runbook missing MCP explain resource"
rg -F -q "wa://incidents/{incident_id}/replay" "${RUNBOOK}" || fail "runbook missing MCP replay resource"
rg -F -q "\`source_failure\`" "${RUNBOOK}" || fail "runbook missing source-failure classification"
rg -F -q "\`policy_denial\`" "${RUNBOOK}" || fail "runbook missing policy-denial classification"
rg -F -q "\`communication_outage\`" "${RUNBOOK}" || fail "runbook missing communication-outage classification"
rg -F -q "\`operator_cancellation\`" "${RUNBOOK}" || fail "runbook missing operator-cancellation classification"
rg -F -q "Outcome is \`infrastructure_block\`" "${RUNBOOK}" || fail "runbook missing RCH infrastructure classification"
rg -F -q "Outcome is \`contaminated_proof_attempt\`" "${RUNBOOK}" || fail "runbook missing dirty-tree classification"
rg -F -q "Outcome is \`source_unavailable\`" "${RUNBOOK}" || fail "runbook missing source-unavailable classification"
rg -F -q "Operator cancellations are distinct from infrastructure failures" "${RUNBOOK}" || fail "runbook missing operator cancellation boundary"
rg -F -q "repair Agent Mail" "${RUNBOOK}" || fail "runbook missing Agent Mail repair prohibition"
rg -F -q "local_cargo_counted false" "${RUNBOOK}" || fail "runbook missing local Cargo citation boundary"
rg -F -q "Do not count that RCH lane unless RCH reports remote execution" "${RUNBOOK}" || fail "runbook missing RCH proof boundary"
rg -F -q "operator-runbook.md#2d-flight-recorder-incident-replay-runbook" "${DOCS_INDEX}" || fail "flight-recorder index missing runbook link"
rg -F -q "operator-runbook.md#2d-flight-recorder-incident-replay-runbook" "${OPERATOR_PLAYBOOK}" || fail "operator playbook missing runbook link"
rg -F -q "operator-runbook.md#2d-flight-recorder-incident-replay-runbook" "${ROOT_README}" || fail "README missing runbook link"
