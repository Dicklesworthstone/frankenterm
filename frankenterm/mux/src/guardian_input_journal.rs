...
use crate::guardian_protocol::{
    GUARDIAN_MAX_INPUT_BYTES, GuardianInputEffectIdentity, InputEffectState,
};
...
/// Exact terminal result produced by consuming one PTY-write permit.
///
/// The fields are private so a caller cannot detach a byte count from the
/// authenticated input identity or manufacture a completion for a retry that
/// never received first-write authority.
#[derive(Debug, Eq, PartialEq)]
#[must_use = "the guardian input write completion must be synchronized before acknowledgement"]
pub struct GuardianInputWriteCompletion {
    identity: GuardianInputEffectIdentity,
    disposition: GuardianInputDisposition,
}
...
impl GuardianInputWritePermit {
    ...
    /// Consume first-write authority after the entire authenticated input was
    /// applied.
    #[must_use]
    pub const fn durable_full(self) -> GuardianInputWriteCompletion {
        GuardianInputWriteCompletion {
            identity: self.identity,
            disposition: GuardianInputDisposition::DurableFull,
        }
    }

    /// Consume first-write authority after one exact nonempty proper prefix
    /// was applied.
    pub fn durable_prefix(
        self,
        applied_bytes: u32,
    ) -> Result<GuardianInputWriteCompletion, GuardianInputJournalError> {
        let disposition = GuardianInputDisposition::DurablePrefix { applied_bytes };
        disposition.validate_for_input_bytes(self.identity.input_bytes())?;
        Ok(GuardianInputWriteCompletion {
            identity: self.identity,
            disposition,
        })
    }

    /// Consume first-write authority only when the writer can prove that zero
    /// input bytes became observable to the child PTY.
    #[must_use]
    pub const fn known_not_applied(self) -> GuardianInputWriteCompletion {
        GuardianInputWriteCompletion {
            identity: self.identity,
            disposition: GuardianInputDisposition::KnownNotApplied,
        }
    }
}

impl GuardianInputWriteCompletion {
    #[must_use]
    pub const fn identity(&self) -> GuardianInputEffectIdentity {
        self.identity
    }
...
impl GuardianInputTerminalPermit {
    ...
    pub(crate) const fn into_protocol_parts(
        self,
    ) -> (GuardianInputEffectIdentity, GuardianInputDisposition) {
        (self.identity, self.receipt.disposition)
    }
}
...
impl GuardianInputJournal {
    ...
    pub fn append_intent_and_sync(
        ...
        if let Some(existing) = self.effects.get(&identity.effect_id()) {
            ...
            if !existing.disposition.is_terminal() {
                return Err(GuardianInputJournalError::EffectIdentityConflict);
            }
            // Protocol receipt rotation may reuse an effect UUID only after
            // the prior incarnation is terminal and the pane's monotonic
            // generation/mux/sequence fence has advanced. The digest chain
            // retains the historical bytes while the current-effect index is
            // replaced by the new exact identity.
            validate_monotonic_input(self.last_input_identity, identity)?;
            return self
                .append_record(identity, GuardianInputDisposition::Intent)
    ...
    /// Synchronize the pre-write acceptance fence for one durable intent.
    pub fn append_acceptance_and_sync(
        &mut self,
        identity: GuardianInputEffectIdentity,
    ) -> Result<GuardianInputJournalAppend, GuardianInputJournalError> {
        self.append_disposition_and_sync(identity, GuardianInputDisposition::AcceptedNotDurable)
    }

    /// Resolve an intent that never reached its acceptance fence and therefore
    /// never authorized a PTY write.
    pub fn resolve_unaccepted_intent_as_known_not_applied_and_sync(
        &mut self,
        identity: GuardianInputEffectIdentity,
    ) -> Result<GuardianInputJournalAppend, GuardianInputJournalError> {
        let existing = self
        ...
        if !matches!(
            ...
            GuardianInputDisposition::Intent | GuardianInputDisposition::KnownNotApplied
        ) {
            return Err(GuardianInputJournalError::InvalidTransition {
                from: existing.disposition,
                to: GuardianInputDisposition::KnownNotApplied,
            });
        }
        self.append_disposition_and_sync(identity, GuardianInputDisposition::KnownNotApplied)
    }

    /// Synchronize one exact state transition. Accepted-to-terminal
    /// transitions are intentionally reachable publicly only through
    /// [`Self::append_write_completion_and_sync`], whose opaque argument
    /// consumes the one first-write permit.
    fn append_disposition_and_sync(
        &mut self,
        identity: GuardianInputEffectIdentity,
        disposition: GuardianInputDisposition,
    ) -> Result<GuardianInputJournalAppend, GuardianInputJournalError> {
    ...
    /// Synchronize the exact terminal result produced by consuming the one
    /// first-write permit for this authenticated input.
    pub fn append_write_completion_and_sync(
        &mut self,
        completion: GuardianInputWriteCompletion,
    ) -> Result<GuardianInputJournalAppend, GuardianInputJournalError> {
        self.append_disposition_and_sync(completion.identity, completion.disposition)
    }

    /// Recover an already synchronized terminal result without issuing new
    /// PTY-write authority.
    pub(crate) fn terminal_protocol_permit(
        &self,
        identity: GuardianInputEffectIdentity,
    ) -> Result<Option<GuardianInputTerminalPermit>, GuardianInputJournalError> {
        let existing = self.exact_effect(identity)?;
        Ok(existing
            .disposition
            .is_terminal()
            .then_some(GuardianInputTerminalPermit {
                identity,
                receipt: existing.receipt(),
            }))
    }

    pub(crate) fn disposition_for_identity(
        &self,
        identity: GuardianInputEffectIdentity,
    ) -> Result<GuardianInputDisposition, GuardianInputJournalError> {
        Ok(self.exact_effect(identity)?.disposition)
    }
    ...
    fn append_record(
        ...
        let previous = self.effects.get(&identity.effect_id()).cloned();
        let starts_reused_effect = disposition == GuardianInputDisposition::Intent
            && previous
                .as_ref()
                .is_some_and(|effect| effect.identity != identity);
        let recovered = advance_effect(
            if starts_reused_effect { None } else { previous },
            identity,
            input_bytes,
            disposition,
            receipt,
...
fn scan_journal(
    ...
    while committed_bytes < physical_bytes {
        ...
        let starts_reused_effect = matches!(
            (existing.as_ref(), disposition),
            (Some(previous), GuardianInputDisposition::Intent)
                if previous.identity != identity && previous.disposition.is_terminal()
        );
        ...
        let recovered = advance_effect(
            if starts_reused_effect { None } else { existing },
            identity,
            input_bytes,
            disposition,
            receipt,
...
mod tests {
    ...
    fn exact_lifecycle_survives_reopen_without_plaintext_fingerprint() {
        ...
        let accepted_append = journal
            .append_acceptance_and_sync(input)
        ...
        let durable_append = journal
            .append_write_completion_and_sync(write_permit.durable_full())
    ...
    fn terminal_retry_after_acknowledgement_loss_is_idempotent() {
        ...
        let terminal_append = journal
            .resolve_unaccepted_intent_as_known_not_applied_and_sync(input)
    ...
    fn durable_prefix_is_exact_bounded_and_idempotent_across_reopen() {
        ...
        journal
            ...
            .expect("activate journal");
        journal.append_intent_and_sync(input).expect("intent");
        let write_permit = journal
            .append_acceptance_and_sync(input)
            .expect("accepted marker")
            .into_first_pty_write_permit()
            .expect("fresh acceptance yields one write permit");
        ...
        let prefix_append = journal
            .append_write_completion_and_sync(
                write_permit
                    .durable_prefix(3)
                    .expect("valid proper prefix completion"),
            )
    ...
    #[cfg(unix)]
    #[test]
    fn terminal_effect_uuid_can_be_reused_only_by_a_new_monotonic_identity() {
        let temp = tempfile::tempdir().expect("create tempdir");
        let path = temp.path().join("input.journal");
        ...
        let second = identity_with_input_bytes(8, 0x73, 0xba, 5);
        let pending_reuse = identity_with_input_bytes(9, 0x73, 0xbb, 4);
        let mut journal = GuardianInputJournal::open(
            create_file(&path),
            pane(),
            cipher(),
        ...
        let first_write = journal
            .append_acceptance_and_sync(first)
            .expect("first acceptance")
            .into_first_pty_write_permit()
            .expect("first acceptance yields write permit");
        journal
            .append_write_completion_and_sync(
                first_write
                    .durable_prefix(2)
            ...
            .expect("first terminal prefix");

        let second_intent = journal
            .append_intent_and_sync(second)
            .expect("terminal UUID may be reused at the next sequence");
        assert!(second_intent.is_newly_committed());
        let second_write = journal
            .append_acceptance_and_sync(second)
            .expect("second acceptance")
            .into_first_pty_write_permit()
            .expect("new identity receives one write permit");
        assert_eq!(second_write.identity(), second);
        assert!(matches!(
            journal.terminal_protocol_permit(first),
            Err(GuardianInputJournalError::EffectIdentityConflict)
        ));
        assert!(matches!(
            journal.append_intent_and_sync(pending_reuse),
            Err(GuardianInputJournalError::EffectIdentityConflict)
        ));
        drop(second_write);
        drop(journal);
        ...
        .expect("recover reused effect UUID");
        let recovered = reopened
            .effect(second.effect_id())
            .expect("current effect incarnation recovered");
        assert_eq!(recovered.identity(), second);
        assert_eq!(
            recovered.disposition(),
            GuardianInputDisposition::AcceptedNotDurable
        );
        assert!(matches!(
            reopened.terminal_protocol_permit(first),
            Err(GuardianInputJournalError::EffectIdentityConflict)
        ));
        let retry = reopened
            .append_acceptance_and_sync(second)
            .expect("current accepted identity reconciles");
        assert!(!retry.is_newly_committed());
        assert!(retry.into_first_pty_write_permit().is_none());
    }
