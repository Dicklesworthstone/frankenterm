...
fn lock_portal_state<'a>(state: &'a Mutex<State>, context: &str) -> MutexGuard<'a, State> {
    state.lock().unwrap_or_else(|poisoned| {
        log::warn!("recovering poisoned xdg-desktop-portal state lock while {context}");
        poisoned.into_inner()
    })
}

fn lock_state(context: &str) -> MutexGuard<'static, State> {
    lock_portal_state(&STATE, context)
}
...
pub fn get_appearance_if_cached() -> anyhow::Result<Option<Appearance>> {
    let state = lock_state("reading cached appearance");
...
pub fn refresh_appearance_in_background() {
    ...
    promise::spawn::spawn(async move {
        ...
        let maybe_notify = {
            let mut state = lock_state("storing refreshed appearance");
...
pub async fn get_appearance() -> anyhow::Result<Option<Appearance>> {
    ...
    match read_setting("org.freedesktop.appearance", "color-scheme").await {
        Ok(value) => {
            ...
            let mut state = lock_state("storing queried appearance");
            state.appearance = CachedAppearance::Some(appearance);
            state.last_update = Instant::now();
            Ok(Some(appearance))
        }
        Err(err) => {
            let mut state = lock_state("storing failed appearance query");
...
pub async fn run_signal_loop(stream: &mut SettingChangedStream<'_>) -> Result<(), anyhow::Error> {
    ...
    {
        let mut state = lock_state("handling initial portal appearance signal state");
    ...
    while let Some(signal) = stream.next().await {
        ...
        if args.namespace == "org.freedesktop.appearance" && args.key == "color-scheme" {
            if let Ok(appearance) = value_to_appearance(args.value) {
                let mut state = lock_state("handling portal appearance signal");
                state.appearance = CachedAppearance::Some(appearance);
                state.last_update = Instant::now();
                drop(state);
                let conn =
...
mod tests {
    ...
    #[test]
    fn portal_state_lock_recovers_after_poison() {
        ...
        let mut recovered = lock_portal_state(&state, "testing poison recovery");
        recovered.appearance = CachedAppearance::Some(Appearance::Light);
        assert_eq!(
            cached_appearance_for_state(&recovered).unwrap(),
