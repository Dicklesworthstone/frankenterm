...
# Configuration
VERBOSE=false
DEFAULT_RCH_CARGO_TARGET_DIR="target/rch-e2e-search-perf-${RUN_ID}"
...
ensure_rch_for_cargo_tests() {
    ...
    mkdir -p "$RCH_LOG_DIR"
    rch_init "$RCH_LOG_DIR" "$RUN_ID" "scripts_search_perf" "$PROJECT_ROOT"
    ensure_rch_ready
    RCH_READY=1
}
...
run_rch_cargo() {
    ...
    local output_file="${RCH_LOG_DIR}/search_perf_${RUN_ID}_step${RCH_STEP_INDEX}.rch.log"
    local rc
