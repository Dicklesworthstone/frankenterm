...
DEFAULT_RCH_CARGO_TARGET_DIR="target/rch-frankensqlite-soak-${RUN_ID}"
...
ensure_rch_for_cargo_tests() {
    ...
    rch_init "$LOG_DIR" "$RUN_ID" "frankensqlite_soak" "$PROJECT_ROOT"
    ensure_rch_ready
    RCH_READY=1
}
...
if run_rch_cargo test -p frankenterm-core --test frankensqlite_perf_tests 2>&1; then
...
if run_rch_cargo test -p frankenterm-core --test frankensqlite_logging_tests 2>&1; then
