...
## [Unreleased] -- `0.15.7`; development on `main` since the local `0.15.2` build

Compare against the latest public release: <https://github.com/Dicklesworthstone/frankenterm/compare/v0.15.1...main>

Version `0.15.7` carries forward the release-candidate fixes below. Publication and final native artifact qualification remain pending.
...
- Mouse selection retains displayed-layout validation across ordinary terminal output and temporary frame contention. Remote mouse delivery keeps ordered work within a bounded queue and retains its admitted worker through scheduler saturation; stale pane identities and ambiguous failures cannot silently redirect input. Native Mac acceptance for forward and reverse drags, Unicode copying, and word/line selection remains pending.
...
- Whole-mux recovery images preserve window titles, ordered tab stacks and their visible members, previous-active-tab history, workspace state, tab layout policy, and domain command policy. Live restoration requires schema 7 and complete topology authority; supported historical images remain available for offline inspection without gaining live-restore authority.
...
- Applying a remote topology snapshot no longer sends its observed dimensions
  back as a new resize command that could undo a newer font or window resize.
...
- Deferred scrollback reads no longer wait for the entire persistence queue to flush. They combine a durable prefix with an immutable pending suffix and reject changed, uncertain or poisoned storage authority before returning text.
- Mouse selection coordinates are tied to complete, successfully submitted pane layouts. Reflow invalidates stale selections even outside the visible viewport, and plain/formatted copying checks layout authority before and after reading text.
- Wrapped-line context expansion uses one bounded budget in both directions. Artificial grouping boundaries preserve significant spaces when copying; rectangular selections retain separate physical rows.
...
- Writer durability: segment and event/gap group commits begin `IMMEDIATE`, so a concurrent commit makes the writer wait instead of dropping the segment with "database is locked". After a dropped segment the shared capture cursor realigns by offset once instead of warning "Sequence discontinuity" on every later segment.
- `ft session recover` no longer refuses crash-orphaned scrollback files with "filename/header identity changed during recovery": the orphan scan sees the raw header (a SIGKILLed writer never flushes its cursor) while the record reader reconciles the cursor, so recovery now compares the file's identity (format, capacity, pane, creation time) rather than writer progress.
- Workspace test suite as a release gate: the CLI contract suite is green again on the fixtures and contracts that had drifted (session topology, structured-error stderr, tx execution exit code); six tx contract tests that still drive sends through the legacy WezTerm CLI stub are ignored with their reason and tracked by ft-ydqah, because pane input over the CLI has failed closed since 0.15.1.
