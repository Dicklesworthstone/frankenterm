...
| Session persistence | Supported with backend prerequisite | Snapshots, session inspection, and `ft session doctor` are implemented cross-platform; live restore still depends on the current WezTerm mux interop boundary. `ft restart` and `ft snapshot restore` are both currently Unix-only (`#[cfg(not(unix))]` emits "currently supported only on Unix platforms" and exits 1 — see main.rs:27100 + :39715). |
...
For inspection without re-verification (pretty-prints the
bundle JSON):
...
Use `--strict-required` on `verify` to fail when the bundle's
`required_categories` list does not match the canonical manifest:
...
ft robot --format json fleet rebalance --strategy load_based --dry-run
```

The live scale and rebalance implementation slices landed under `ft-2jlme`
and `ft-v07yg` respectively. Scale computes native spawn/stop plans and
commits through the mux-backed fleet mutation substrate; rebalance computes
work-assignment plans and commits through the `work_claims` mutation
substrate. Dry-run uses the same plan path and returns a `dry_run` receipt
without side effects. Unhealthy inventory, work-queue, policy, approval,
planning, and mutation failures use typed `robot.fleet.*` error envelopes.
...
resource-pressure cockpit contract separates `rust_heap`, `mmap_file_backed`,
`sqlite_page_cache`, `graphics_media`, `scrollback_cache`, `child_processes`,
and `unknown` resident bytes; see `docs/operator-playbook.md` for the collection
flow and `docs/resource-pressure-cockpit-contract.md` for the proof gate and
...
ft tx plan --contract-file tx.json
