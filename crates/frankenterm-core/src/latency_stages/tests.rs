...
fn test_parallel_composition_max() {
    let a = StageBudget::new(LatencyStage::PtyCapture, 100.0, 200.0, 300.0, 400.0).unwrap();
    let b = StageBudget::new(LatencyStage::DeltaExtraction, 150.0, 180.0, 250.0, 500.0).unwrap();
...
fn test_conditional_with_else_branch() {
    ...
    .unwrap();
    let else_b = StageBudget::new(LatencyStage::ApiResponse, 200.0, 400.0, 600.0, 1000.0).unwrap();
    let cond = BudgetNode::Cond {
        probability: 0.3,
        then_branch: Box::new(BudgetNode::Leaf(then_b)),
...
fn test_allocator_degradation_display() {
    ...
    assert!(
        format!("{}", AllocatorDegradation::Oscillating { lane_count: 5 }).contains("OSCILLATING")
    );
    assert!(
        format!(
            "{}",
...
fn test_eprocess_peak_e_value() {
    ...
    assert!(
        det.peak_e_value() >= peak_after_up || (det.peak_e_value() - peak_after_up).abs() < 1e-10
    );
}
