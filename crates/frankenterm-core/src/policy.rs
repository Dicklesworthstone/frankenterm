...
//!
//! Provides capability gates, authorization decisions, and rate limiting.
//!
//! # Architecture
//!
...
pub enum ConnectorOperationDispatchError {
    #[error("connector operation denied: {reason}")]
    Denied { reason: String },
    #[error("connector mesh routing failed: {reason}")]
    Mesh { reason: String, retryable: bool },
    #[error("connector host runtime failed: {reason}")]
    HostRuntime {
        reason: String,
        failure_class: crate::connector_host_runtime::ConnectorFailureClass,
        retryable: bool,
    },
}

impl ConnectorOperationDispatchError {
    fn from_mesh_error(err: crate::connector_mesh::ConnectorMeshError) -> Self {
        let reason = err.to_string();
        let retryable = matches!(
            err,
            crate::connector_mesh::ConnectorMeshError::NoHealthyHosts { .. }
                | crate::connector_mesh::ConnectorMeshError::RoutingFailed { .. }
                | crate::connector_mesh::ConnectorMeshError::CapacityExceeded { .. }
                | crate::connector_mesh::ConnectorMeshError::PartitionDetected { .. }
        );
        Self::Mesh { reason, retryable }
    }

    fn from_host_runtime_error(
        ...
        let failure_class = match &err {
            ...
            crate::connector_host_runtime::ConnectorHostRuntimeError::BudgetExceeded { .. } => {
                crate::connector_host_runtime::ConnectorFailureClass::Quota
            }
            crate::connector_host_runtime::ConnectorHostRuntimeError::SandboxViolation {
                ..
            } => crate::connector_host_runtime::ConnectorFailureClass::Policy,
            ...
            | crate::connector_host_runtime::ConnectorHostRuntimeError::ProtocolUpgradeRejected {
                ..
            } => crate::connector_host_runtime::ConnectorFailureClass::Validation,
            crate::connector_host_runtime::ConnectorHostRuntimeError::HostNotRunnable {
                ..
            } => crate::connector_host_runtime::ConnectorFailureClass::Unknown,
        };
        let reason = err.to_string();
        let retryable = matches!(
            err,
    ...
    pub const fn is_retryable(&self) -> bool {
        match self {
            Self::Denied { .. } => false,
            Self::Mesh { retryable, .. } | Self::HostRuntime { retryable, .. } => *retryable,
        }
    }

    #[must_use]
    pub const fn failure_class(&self) -> crate::connector_host_runtime::ConnectorFailureClass {
        match self {
            Self::Denied { .. } => crate::connector_host_runtime::ConnectorFailureClass::Policy,
            Self::Mesh { retryable, .. } => {
                if *retryable {
                    crate::connector_host_runtime::ConnectorFailureClass::Network
                } else {
                    crate::connector_host_runtime::ConnectorFailureClass::Validation
                }
            }
...
impl PolicyEngine {
    ...
    /// Production operator boundary for connector lifecycle management
    /// (ft-7h5da.5.11).
    ///
    /// This is the single gated entry point that drives connector
    /// install/update/enable/disable/restart/uninstall/rollback intents
    /// through the owned [`ConnectorLifecycleManager`], so lifecycle telemetry
    /// (`op_counter`) reflects real operator-driven operations rather than the
    /// initialized-but-idle state that diagnostics would otherwise report.
    ///
    ...
    ///
    /// [`ConnectorLifecycleManager`]: crate::connector_lifecycle::ConnectorLifecycleManager
    pub fn run_connector_lifecycle_intent(
        ...
        self.authorize_connector_lifecycle_kill_switch(
            intent.op_name(),
            intent.connector_id(),
            now_ms,
        )?;
        ...
        match self.lifecycle_manager_mut().execute(intent, now_ms) {
            Ok(outcome) => {
                tracing::info!(
                    connector_id = %connector_id,
                    op,
                    success = outcome.success,
                    admin_state = ?outcome.admin_state,
    ...
    pub fn route_connector_operation_through_mesh(
        ...
        let routing_request = crate::connector_mesh::RoutingRequest {
            ...
            required_capabilities: vec![request.capability],
            preferred_zone: Some(host_config.sandbox.zone_id.clone()),
    ...
    C: crate::wezterm::WeztermInterface,
{
    ...
    async fn inject_with_cx(
        ...
        let effect_fence = if let Some(storage) = self.storage.as_ref() {
            ...
            self.kill_switch_freshness
                .apply(&mut self.engine, loaded, checked_now_ms_u64());
            fence.ok()
        } else {
            None
        };
        ...
        C: Clone + 'a,
    {
        ...
        let effect_fence = if let Some(storage) = self.storage.as_ref() {
            use crate::policy_kill_switch_state::{
                KillSwitchStateError, acquire_kill_switch_fence,
                load_kill_switch_state_from_path_with_cx,
            };
            let fence = acquire_kill_switch_fence(std::path::Path::new(storage.db_path()));
            if matches!(&fence, Err(KillSwitchStateError::FencePending)) {
                transient_switch = Some(self.engine.kill_switch_state().clone());
            ...
            let loaded = match &fence {
                Ok(guard) => self
                    .kill_switch_freshness
                    .bind(guard)
                    .and_then(|()| load_kill_switch_state_from_path_with_cx(cx, storage.db_path())),
                Err(error) => Err(error.clone()),
            };
            self.kill_switch_freshness
                .apply(&mut self.engine, loaded, checked_now_ms_u64());
...
mod tests {
    ...
    async fn persisted_stop_injector() -> (
        ...
        let cx = crate::cx::for_request();
        let storage = crate::storage::StorageHandle::new_with_cx(&cx, path.to_str().unwrap())
            .await
            .unwrap();
        ...
        storage
            .upsert_pane_with_cx(
                ...
                crate::storage::PaneRecord {
                    ...
                    observed: true,
                    ignore_reason: None,
                    last_decision_at: Some(1000),
                },
            )
            .await
            .unwrap();
        let injector =
            PolicyGatedInjector::with_storage(PolicyEngine::permissive(), mock, storage.clone());
    ...
    fn persisted_stop_injector_observes_later_process_trip_and_fresh_reset() {
        run_async_test(async {
            ...
            let baseline = injector
                .send_text_with_cx(&cx, 42, "echo baseline\n", ActorKind::Workflow, &caps, None)
                .await;
            assert!(
                matches!(
                    baseline,
                    InjectionResult::Allowed {
                        audit_action_id: Some(_),
                        ..
                    }
                ),
                "{baseline:?}"
            ...
            for workflow in ["queued-before-trip", "queued-after-trip"] {
                ...
                let InjectionResult::Denied {
                    decision,
                    audit_action_id: Some(audit_id),
                    ..
                } = denied
                else {
                    panic!("later trip must be audited and denied: {denied:?}");
                };
                assert_eq!(decision.rule_id(), Some("policy.kill_switch"));
                let rows = storage
                    .get_audit_actions_with_cx(
                        &cx,
                        crate::storage::AuditQuery {
                            pane_id: Some(42),
                            ..Default::default()
                        },
                    )
                ...
                assert!(
                    rows.iter()
                        .any(|row| row.id == audit_id && row.policy_decision == "deny")
                );
                assert_eq!(
                    injector.client.pane_state(42).await.unwrap().content,
                    before
                );
            }
            ...
            assert_eq!(
                injector.client.pane_state(42).await.unwrap().content,
                before,
                "reset must not replay rejected sends"
            );
            let recovered = injector
                .send_text_with_cx(
                    &cx,
                    42,
                    "echo fresh-authorized\n",
                    ActorKind::Workflow,
                    &caps,
                    None,
                )
    ...
    fn persisted_stop_injector_corrupt_read_and_cancel_fail_without_pane_input() {
        run_async_test(async {
            ...
            let before = injector.client.pane_state(42).await.unwrap().content;
            storage
                .set_config_value_with_cx(
                    &cx,
                    crate::policy_kill_switch_state::KILL_SWITCH_STATE_KEY,
                    "{broken",
                )
                .await
                .unwrap();
            ...
            let unreadable = injector
                ...
                .await;
            assert!(
                matches!(unreadable, InjectionResult::Denied { .. }),
                "{unreadable:?}"
            );
            let cancelled = crate::cx::for_request();
            cancelled.cancel_with(
                crate::outcome::CancelKind::User,
                Some("owned cancellation regression"),
            );
            let result = injector
                .send_text_with_cx(
                    &cancelled,
    ...
    fn persisted_stop_injector_actual_workflow_wrapper_refreshes_and_audits() {
        run_async_test(async {
            ...
            let wrapper = crate::workflows::CxPolicyInjector::new(
            ...
            let denied = wrapper
                ...
                .unwrap();
            assert!(
                matches!(denied, InjectionResult::Denied { ref decision, audit_action_id: Some(_), .. } if decision.rule_id() == Some("policy.kill_switch")),
                "{denied:?}"
            );
            let control = wrapper
                .send_ctrl_c(&cx, 42, ActorKind::Workflow, &caps, None)
                .await
                .unwrap();
            assert!(
                matches!(
    ...
    fn persisted_stop_injector_detached_queue_owns_fence_and_pending_is_not_latched() {
        run_async_test(async {
            ...
            assert!(
                transition_in_test_process(std::path::Path::new(storage.db_path()), "pending")
                    .contains("KILL_SWITCH_CHILD_PENDING")
            );
            ...
            let denied = injector
                ...
                .await;
            assert!(
                matches!(denied, InjectionResult::Denied { .. }),
                "{denied:?}"
            );
            assert_eq!(
                injector.engine().kill_switch_state().level,
                crate::policy_quarantine::KillSwitchLevel::Disarmed
            );
            drop(queued);
            assert!(
                mock.pane_state(42).await.unwrap().content.is_empty(),
                "dropping a queued future must not dispatch"
            );
    ...
    #[test]
    fn run_connector_lifecycle_intent_production_boundary_drives_telemetry() {
        // ft-7h5da.5.11: the production operator boundary must drive the owned
        // ConnectorLifecycleManager so its `op_counter` telemetry advances from
        // a real production call path (PolicyEngine::run_connector_lifecycle_intent),
        // not just from initialized-but-idle state. This is the non-test
        // production path that operator/CLI/runtime surfaces route through.
        let mut engine = PolicyEngine::permissive();
        let before = engine.lifecycle_manager().op_counter();

        // Enable on a not-yet-installed connector still drives the manager
        // (op_counter increments before the per-intent match) and surfaces a
        // real lifecycle error instead of silently succeeding on idle state.
        let result = engine.run_connector_lifecycle_intent(
            crate::connector_lifecycle::LifecycleIntent::Enable {
        ...
        assert_eq!(
            engine.lifecycle_manager().op_counter(),
            before + 1,
            "production boundary must drive lifecycle-manager telemetry (op_counter)"
        );
        assert!(
            result.is_err(),
            "enabling a non-installed connector should surface a real lifecycle error"
        );
    }
    ...
    fn route_connector_operation_through_mesh_plans_without_host_or_mesh_mutation() {
        ...
        engine
            ...
            .register_zone(crate::connector_mesh::MeshZone::new(
                host.sandbox.zone_id.clone(),
                "explicit model fixture",
            ))
            .unwrap();
        engine
            .connector_mesh_mut()
        ...
        let result = engine
            .route_connector_operation_through_mesh(
                "slack",
                crate::connector_host_runtime::ConnectorOperationRequest::new(
                    "connector.slack.invoke",
                    "corr-mesh-1",
                    crate::connector_host_runtime::ConnectorCapability::Invoke,
                ),
                10_000,
            )
