...
mod tests {
    ...
    fn list_tag_filter_filters_in_process() {
        ...
        let v = handle_profile_command("list", &json!({ "tag_filter": "x" }), &conn)
            .expect("ok");
        let arr = v.get("profiles").and_then(Value::as_array).unwrap();
        assert_eq!(arr.len(), 1);
        assert_eq!(
    ...
    fn show_returns_full_profile_data() {
        ...
        let v = handle_profile_command("show", &json!({ "name": "p1" }), &conn)
            .expect("ok");
    ...
    fn show_for_missing_profile_returns_not_found() {
        let conn = fresh_conn();
        let err = handle_profile_command("show", &json!({ "name": "ghost" }), &conn)
            .unwrap_err();
        match err {
    ...
    fn validate_returns_valid_for_well_formed_profile() {
        ...
        let v = handle_profile_command("validate", &json!({ "name": "good" }), &conn)
            .expect("ok");
    ...
    fn validate_returns_invalid_with_reason_when_missing() {
        let conn = fresh_conn();
        let v = handle_profile_command("validate", &json!({ "name": "ghost" }), &conn)
            .expect("ok");
    ...
    fn apply_dry_run_returns_plan_without_mutation() {
        ...
        .expect("ok");
        assert_eq!(
            v.get("profile_name").and_then(Value::as_str),
            Some("ready")
        );
        assert_eq!(v.get("dry_run").and_then(Value::as_bool), Some(true));
        assert_eq!(
            v.get("panes_spawned").and_then(Value::as_array).unwrap().len(),
            0
        );
    }
    ...
    fn apply_non_dry_run_returns_spawn_not_wired() {
        let conn = fresh_conn();
        insert_agent_profile(&conn, &synth("ready", "r", vec![])).unwrap();
        let err = handle_profile_command("apply", &json!({ "name": "ready" }), &conn)
            .unwrap_err();
        match err {
    ...
    fn apply_for_missing_profile_returns_not_found_before_spawn_check() {
        let conn = fresh_conn();
        let err = handle_profile_command("apply", &json!({ "name": "ghost" }), &conn)
            .unwrap_err();
