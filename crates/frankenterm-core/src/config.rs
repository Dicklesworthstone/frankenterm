...
pub struct SessionRetentionConfig {
    ...
    /// Maximum exact logical retained-session payload in MiB (0 = unlimited).
    ///
    /// Schema v40 charges every non-NULL INTEGER as eight bytes and every
    /// TEXT/BLOB by its encoded byte length across `mux_sessions`,
    /// `session_checkpoints`, `mux_pane_state`, and
    /// `restore_attempt_lifecycle`. Each stored field is charged exactly once;
    /// session topology is not multiplied by checkpoint cardinality. SQLite
    /// record framing, b-tree/index pages, freelist pages, WAL bytes, and
    /// unrelated tables remain outside this explicitly logical budget.
