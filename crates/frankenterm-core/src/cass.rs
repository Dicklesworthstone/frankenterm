...
// ft-8cg6y / ft-y0loj.5.D.1: cass plain-data types extracted to
// `frankenterm-core-cass-types`. Re-export so existing
// `crate::cass::*` and `frankenterm_core::cass::*` paths resolve unchanged.
pub use frankenterm_core_cass_types::{
    CassAgent, CassContentExportQuery, CassContextLine, CassError, CassExportContentChunk,
    CassExportQuery, CassExportSessionRecord, CassIndexResult, CassMessage, CassSearchHit,
    CassSearchResult, CassSession, CassSessionSummary, CassStatus, CassViewResult, SearchOptions,
    ViewOptions, parse_cass_timestamp_ms,
};

pub trait CassAgentProviderExt: Sized {
    fn from_provider(provider: &AgentProvider) -> Option<Self>;
    fn to_provider(self) -> AgentProvider;
}

impl CassAgentProviderExt for CassAgent {
    fn from_provider(provider: &AgentProvider) -> Option<Self> {
        match provider {
            ...
            AgentProvider::Unknown(slug) if CassAgent::from_slug(slug) == Some(Self::ChatGpt) => {
                Some(Self::ChatGpt)
            }
            _ => None,
        }
...
pub trait CassErrorRemediationExt {
    fn remediation(&self) -> Remediation;
}

impl CassErrorRemediationExt for CassError {
