...
impl DecisionEvent {
    ...
    #[must_use]
    // The constructor mirrors the persisted event schema; callers pass the
    // schema fields explicitly so replay fixtures stay easy to audit.
    ...
    pub fn new(
        ...
        // lines of pure UTF-8 boundary handling — duplicating it here
        // is cheaper than introducing a back-edge to core.
        let input_summary = {
            ...
            if input_text.len() <= MAX_BYTES {
                ...
                while end > 0 && !input_text.is_char_boundary(end) {
                    end -= 1;
                }
                input_text[..end].to_string()
            }
        };
...
mod tests {
    ...
    fn nodes_by_type_absent() {
        ...
        assert_eq!(noop, [] as [&crate::replay_decision_graph::DecisionNode; 0]);
    }
