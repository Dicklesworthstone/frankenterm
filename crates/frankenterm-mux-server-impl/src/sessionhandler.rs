...
fn reliable_pane_write_claim_outcome(
    ...
    match outcome {
        ...
        ReliableInputClaimOutcome::DuplicateApplied(ReliableInputAppliedEffect::KeyEvent) => Err(
            input_rejection(ReliableKeyEventRejectionV1::IdentityConflict),
        ),
        ReliableInputClaimOutcome::DuplicatePending => Err(ReliablePaneWriteOutcomeV1::Retry(
            ReliablePaneWriteRetryV1::Input(ReliableKeyEventRetryV1::DuplicatePending {
                retry_after_ns: RELIABLE_INPUT_RETRY_AFTER_NS,
            }),
        )),
        ReliableInputClaimOutcome::OutcomeUnknown => Err(ReliablePaneWriteOutcomeV1::Indeterminate),
        ReliableInputClaimOutcome::ClientNotRegistered
        | ReliableInputClaimOutcome::ClientRegistrationRetired => Err(
...
#[derive(Default)]
struct PanePushQueue {
    pending: HashMap<PaneId, PendingPanePush>,
    order: VecDeque<PaneId>,
    closed: bool,
}

struct PanePushCoordinator {
    queue: Mutex<PanePushQueue>,
    wake: frankenterm_core::runtime_async::mpsc::Sender<()>,
}

impl PanePushCoordinator {
    fn lock_queue(&self) -> std::sync::MutexGuard<'_, PanePushQueue> {
            ...
            Err(error) => {
                ...
                for (_, pending) in queue.pending.drain() {
                    pending
                        .tracked
                        .push_state
                        .store(PANE_PUSH_IDLE, Ordering::Release);
                }
                queue.order.clear();
    ...
    fn enqueue(&self, pane_id: PaneId, pending: PendingPanePush) {
        let mut queue = self.lock_queue();
        if queue.closed || !pending.tracked.live.load(Ordering::Acquire) {
            pending
                .tracked
                .push_state
                .store(PANE_PUSH_IDLE, Ordering::Release);
        ...
        if !queue.pending.contains_key(&pane_id) {
            queue.order.push_back(pane_id);
            queue.pending.insert(pane_id, pending);
        }
        drop(queue);
        // A full one-element wake channel already represents all pending bits.
        let _ = self.wake.try_send(());
    }

    fn remove(&self, pane_id: PaneId) {
        let mut queue = self.lock_queue();
        if let Some(pending) = queue.pending.remove(&pane_id) {
            pending
                .tracked
                .push_state
                .store(PANE_PUSH_IDLE, Ordering::Release);
            queue.order.retain(|id| *id != pane_id);
        }
        drop(queue);
        let _ = self.wake.try_send(());
    }
    ...
    async fn run(
        self: Arc<Self>,
        mut wake: frankenterm_core::runtime_async::mpsc::Receiver<()>,
        sender: PduSender,
        authority: SessionAuthority,
    ) {
        loop {
            let (next, delay) = {
                ...
                for _ in 0..queue.order.len() {
                    ...
                    let Some(pending) = queue.pending.get(&id) else {
                        continue;
                    };
                    if pending.ready_at <= now {
                        next = queue.pending.remove(&id).map(|pending| (id, pending));
                        break;
                    }
                    let remaining = pending.ready_at.duration_since(now);
                    delay = Some(delay.map_or(remaining, |current: std::time::Duration| {
            ...
            let Some((pane_id, mut pending)) = next else {
                let cx = frankenterm_core::cx::Cx::current()
                    .unwrap_or_else(frankenterm_core::cx::for_request);
                if let Some(delay) = delay {
                    let message = wake.recv(&cx);
                    let timer = frankenterm_core::runtime_async::sleep(delay);
                    futures::pin_mut!(message, timer);
                    let _ = futures::future::select(message, timer).await;
                } else if wake.recv(&cx).await.is_err() {
                    return;
                }
                continue;
            };
            let Some(registration) = pending.tracked.registration.as_ref() else {
                continue;
            };
            let mut guard = PanePushStateGuard {
                state: Arc::clone(&pending.tracked.push_state),
                disarmed: false,
            };
            if !pending.tracked.live.load(Ordering::Acquire) {
                continue;
            }
            pending
                .tracked
                .push_state
                .store(PANE_PUSH_SCHEDULED, Ordering::Release);
                ...
                registration
                    .try_with_current(|pane| {
                        maybe_push_pane_changes(
                            &pane,
                            sender.clone(),
                            Arc::clone(&pending.tracked.state),
                        )
                    })
            ...
            let retry = match result {
                Ok(Ok(())) => {
                    pending.retry_delay = std::time::Duration::from_millis(1);
                    ...
                    ) =>
                {
                    ...
                    metrics::counter!("mux.server.detached_render_capture_retry", "cause" => cause)
                        .increment(1);
                    pending.ready_at = Instant::now() + pending.retry_delay;
                    pending.retry_delay =
                        (pending.retry_delay * 2).min(std::time::Duration::from_millis(16));
                    true
                }
                Ok(Err(err)) | Err(err) => {
            ...
            if retry {
                self.enqueue(pane_id, pending);
                guard.disarmed = true;
            }
            drop(guard);
            // One pane per poll: busy or continuously noisy panes cannot
            // monopolize this session owner or overtake admitted input.
...
struct PanePushCoordinatorGuard {
    coordinator: Arc<PanePushCoordinator>,
    authority: SessionAuthority,
}

impl Drop for PanePushCoordinatorGuard {
    fn drop(&mut self) {
        self.authority.retire();
...
const PANE_SNAPSHOT_METADATA_ATTEMPT_BYTES: usize = 4 * 1024 * 1024;
const PANE_SNAPSHOT_METADATA_REQUEST_BYTES: usize =
    PANE_SNAPSHOT_METADATA_ATTEMPT_BYTES * COHERENT_SNAPSHOT_ATTEMPTS as usize;

fn new_pane_snapshot_metadata_ledger() -> anyhow::Result<mux::tab::PaneSnapshotMetadataLedger> {
    use mux::tab::PaneSnapshotMetadataLimits;

    // Field order is the frozen PaneSnapshotMetadataField::ALL order. These
    // ceilings protect the producer allocation boundary; the aggregate body
    // codec limit remains a separate final invariant.
    let limits = PaneSnapshotMetadataLimits::new(
        [
            64 * 1024,  // window title
            16 * 1024,  // window workspace source
            64 * 1024,  // tab title
            64 * 1024,  // pane title
            256 * 1024, // pane working-directory URL
            16 * 1024,  // pane workspace copy
            16 * 1024,  // pane tty name
        ],
        PANE_SNAPSHOT_METADATA_ATTEMPT_BYTES,
        PANE_SNAPSHOT_METADATA_ATTEMPT_BYTES,
        PANE_SNAPSHOT_METADATA_REQUEST_BYTES,
        PANE_SNAPSHOT_METADATA_REQUEST_BYTES,
    );
...
struct PaneSnapshotMetadataAttempt<'a> {
    ledger: &'a mut mux::tab::PaneSnapshotMetadataLedger,
    checkpoint: mux::tab::PaneSnapshotMetadataStats,
    armed: bool,
}

impl<'a> PaneSnapshotMetadataAttempt<'a> {
    fn new(ledger: &'a mut mux::tab::PaneSnapshotMetadataLedger) -> Self {
        Self {
            checkpoint: ledger.attempt_checkpoint(),
            ledger,
            armed: true,
        }
    }
...
impl std::ops::Deref for PaneSnapshotMetadataAttempt<'_> {
    type Target = mux::tab::PaneSnapshotMetadataLedger;
...
impl std::ops::DerefMut for PaneSnapshotMetadataAttempt<'_> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        self.ledger
    }
}

impl Drop for PaneSnapshotMetadataAttempt<'_> {
    fn drop(&mut self) {
        if self.armed {
            if let Err(error) = self.ledger.release_attempt_to(self.checkpoint) {
                log::error!("pane-snapshot metadata attempt rollback failed: {error}");
            }
        }
    }
}
...
fn record_pane_snapshot_metadata_metrics(
    family: &'static str,
    ledger: &mut mux::tab::PaneSnapshotMetadataLedger,
) {
    for (field, admitted_values) in ledger.take_unreported_admitted_values() {
        metrics::counter!(
            "mux.server.pane_snapshot_metadata.admitted_values_total",
            "family" => family,
            "field" => field.label(),
        )
        .increment(u64::try_from(admitted_values).unwrap_or(u64::MAX));
    }
    for (scope, stats) in [
        ("attempt", ledger.attempt_stats()),
        ("request", ledger.request_stats()),
    ] {
        for field in mux::tab::PaneSnapshotMetadataField::ALL {
            let usage = stats.field(field);
            metrics::histogram!(
                "mux.server.pane_snapshot_metadata.retained_bytes",
                "family" => family,
                "scope" => scope,
                "field" => field.label(),
            )
            .record(usage.retained_bytes as f64);
            metrics::histogram!(
                "mux.server.pane_snapshot_metadata.encoded_bytes",
                "family" => family,
                "scope" => scope,
                "field" => field.label(),
            )
            .record(usage.encoded_bytes as f64);
        }
        let total = stats
            .total()
            .unwrap_or(mux::tab::PaneSnapshotMetadataUsage {
                values: usize::MAX,
                retained_bytes: usize::MAX,
                encoded_bytes: usize::MAX,
            });
        metrics::histogram!(
            "mux.server.pane_snapshot_metadata.total_retained_bytes",
            "family" => family,
            "scope" => scope,
        )
        .record(total.retained_bytes as f64);
        metrics::histogram!(
            "mux.server.pane_snapshot_metadata.total_encoded_bytes",
            "family" => family,
            "scope" => scope,
        )
        .record(total.encoded_bytes as f64);
    }
    let request_high_water =
        ledger
            .request_stats()
            .total()
            .unwrap_or(mux::tab::PaneSnapshotMetadataUsage {
    ...
    for (scope, retained_bytes, encoded_bytes) in [
        (
            "attempt",
            ledger.attempt_peak_retained_bytes(),
            ledger.attempt_peak_encoded_bytes(),
        ),
        (
            "request",
            request_high_water.retained_bytes,
            request_high_water.encoded_bytes,
        ),
    ] {
        metrics::histogram!(
            "mux.server.pane_snapshot_metadata.retained_bytes_high_water",
            "family" => family,
            "scope" => scope,
        )
        .record(retained_bytes as f64);
        metrics::histogram!(
            "mux.server.pane_snapshot_metadata.encoded_bytes_high_water",
            "family" => family,
            "scope" => scope,
        )
    ...
    if let Some(rejection) = ledger.last_rejection() {
        let (reason, field) = match rejection {
            mux::tab::PaneSnapshotMetadataRejection::FieldLimit { field } => ("field_limit", field),
            mux::tab::PaneSnapshotMetadataRejection::ArithmeticOverflow => {
                ("arithmetic", "aggregate")
            }
            mux::tab::PaneSnapshotMetadataRejection::AttemptRetainedLimit => {
                ("attempt_retained", "aggregate")
            }
            mux::tab::PaneSnapshotMetadataRejection::AttemptEncodedLimit => {
                ("attempt_encoded", "aggregate")
            }
            mux::tab::PaneSnapshotMetadataRejection::RequestRetainedLimit => {
                ("request_retained", "aggregate")
            }
            mux::tab::PaneSnapshotMetadataRejection::RequestEncodedLimit => {
                ("request_encoded", "aggregate")
            }
        };
        metrics::counter!(
            "mux.server.pane_snapshot_metadata.rejections",
            "family" => family,
            "reason" => reason,
            "field" => field,
        )
    ...
    record_pane_snapshot_metadata_retry_release(family, ledger);
}

fn record_pane_snapshot_metadata_retry_release(
    family: &'static str,
    ledger: &mut mux::tab::PaneSnapshotMetadataLedger,
) {
    let released = ledger.take_retry_released_bytes();
    if released == 0 {
        return;
    }
    metrics::counter!(
        "mux.server.pane_snapshot_metadata.retry_released_bytes_total",
        "family" => family,
    )
...
fn budgeted_floating_pane_snapshot_entry(
    window_id: mux::window::WindowId,
    tab_id: mux::tab::TabId,
    workspace: &str,
    positioned: mux::tab::PositionedFloatingPane,
    census_ledger: &mut mux::tab::PaneSnapshotCensusLedger,
    metadata_ledger: &mut mux::tab::PaneSnapshotMetadataLedger,
) -> anyhow::Result<codec::FloatingPaneSnapshotEntry> {
    census_ledger.preflight_pane_callbacks(6)?;
    metadata_ledger.preflight_required_string(
        mux::tab::PaneSnapshotMetadataField::PaneWorkspace,
        workspace,
    )?;
    ...
    metadata_ledger
        .preflight_required_string(mux::tab::PaneSnapshotMetadataField::PaneTitle, "")?;
    let title =
        observe_floating_snapshot_pane_callback(&pane, census_ledger, |pane| pane.get_title())?;
    metadata_ledger.admit_required_owned(
        mux::tab::PaneSnapshotMetadataField::PaneTitle,
        &title,
        title.capacity(),
    )?;
    let dimensions = observe_floating_snapshot_pane_callback(&pane, census_ledger, |pane| {
        pane.get_dimensions()
    })?;
    metadata_ledger
        .preflight_optional_value(mux::tab::PaneSnapshotMetadataField::PaneWorkingDir)?;
    let working_dir = observe_floating_snapshot_pane_callback(&pane, census_ledger, |pane| {
        pane.get_current_working_dir(CachePolicy::AllowStale)
    })?;
    if let Some(url) = working_dir.as_ref() {
        metadata_ledger.preflight_optional_string(
            mux::tab::PaneSnapshotMetadataField::PaneWorkingDir,
            url.as_str(),
        )?;
    }
    let working_dir = working_dir.map(mux::tab::SerdeUrl::from);
    match working_dir.as_ref() {
        ...
        None => metadata_ledger
            .admit_optional_none(mux::tab::PaneSnapshotMetadataField::PaneWorkingDir)?,
    }
    let alt_screen_active =
        observe_floating_snapshot_pane_callback(&pane, census_ledger, |pane| {
            pane.is_alt_screen_active()
        })?;
    let cursor_pos = observe_floating_snapshot_pane_callback(&pane, census_ledger, |pane| {
        pane.get_cursor_position()
    })?;
    metadata_ledger.preflight_optional_value(mux::tab::PaneSnapshotMetadataField::PaneTtyName)?;
    let tty_name =
        observe_floating_snapshot_pane_callback(&pane, census_ledger, |pane| pane.tty_name())?;
    match tty_name.as_ref() {
        Some(tty_name) => metadata_ledger.admit_optional_owned(
            mux::tab::PaneSnapshotMetadataField::PaneTtyName,
            tty_name,
            tty_name.capacity(),
        )?,
        None => {
    ...
    Ok(codec::FloatingPaneSnapshotEntry {
...
fn observe_floating_snapshot_pane_callback<T>(
    pane: &Arc<dyn mux::pane::Pane>,
    ledger: &mut mux::tab::PaneSnapshotCensusLedger,
    callback: impl FnOnce(&Arc<dyn mux::pane::Pane>) -> T,
) -> anyhow::Result<T> {
    ledger.reserve_pane_callbacks(1)?;
    catch_recoverable(
        RecoverablePanicSite::MuxPaneCallback,
...
fn append_floating_pane_snapshot(
    ...
    for positioned in positioned {
        let entry = budgeted_floating_pane_snapshot_entry(
            window_id,
            tab.tab_id(),
            workspace,
            positioned,
...
async fn collect_list_panes_snapshot_with_stage_observer_and_census(
    ...
    *cardinality = PaneSnapshotCardinalityStats::default();
    ...
    for window_id in window_ids {
        ...
        metadata_ledger.release_retained_only(
            mux::tab::PaneSnapshotMetadataField::WindowWorkspace,
            workspace.capacity(),
        )?;
        drop(workspace);
...
async fn collect_coherent_list_panes_snapshot_with_stage_observer(
    ...
    for attempt in 1..=COHERENT_SNAPSHOT_ATTEMPTS {
        ...
        drop(panes);
        metadata_ledger.release_attempt_to(Default::default())?;
        record_pane_snapshot_metadata_retry_release("pdu82", &mut metadata_ledger);
        metrics::counter!(
            "mux.server.coherent_snapshot.total",
            "outcome" => "retry"
        )
...
async fn collect_ordered_list_panes_snapshot_with_stage_observer(
    ...
    for attempt in 1..=COHERENT_SNAPSHOT_ATTEMPTS {
        ...
        let collection = async {
            for (frozen, ordered_window) in frozen_windows.iter_mut().zip(&ordered_windows) {
                ...
                pane_window_titles.push(mux::tab::PaneArenaWindowTitle {
                    ...
                    title: std::mem::take(&mut frozen.title),
                });
                ...
                let workspace = std::mem::take(&mut frozen.workspace);
                metadata_ledger.release_retained_only(
                    mux::tab::PaneSnapshotMetadataField::WindowWorkspace,
                    workspace.capacity(),
                )?;
        ...
        drop(pane_trees);
        drop(pane_nodes);
        drop(pane_window_titles);
        drop(floating_panes);
        drop(ordered_windows);
        drop(frozen_windows);
        metadata_ledger.release_attempt_to(Default::default())?;
        record_pane_snapshot_metadata_retry_release("pdu87", &mut metadata_ledger);
        metrics::counter!(
            "mux.server.ordered_snapshot.total",
            "outcome" => "retry"
        )
...
impl SessionHandler {
    ...
    pub fn new_for_mux(to_write_tx: PduSender, mux: Arc<Mux>) -> anyhow::Result<Self> {
    ...
    pub(crate) fn new_for_session_with_topology_stream(
        ...
        let reservation = match try_reserve_main_thread_with_low_priority(
            ...
            rejected => anyhow::bail!("cannot admit session render coordinator: {rejected:?}"),
        };
        let (wake_tx, wake_rx) = frankenterm_core::runtime_async::mpsc::channel(1);
        let coordinator = Arc::new(PanePushCoordinator {
            queue: Mutex::new(PanePushQueue::default()),
            wake: wake_tx,
        });
        let guard = PanePushCoordinatorGuard {
            coordinator: Arc::clone(&coordinator),
            authority: owner.authority(),
        };
        let worker = Arc::clone(&coordinator);
        let authority = owner.authority();
        let sender = to_write_tx.clone();
    ...
    fn new(to_write_tx: PduSender) -> Self {
        ...
        // State-only unit tests exercise synchronous helpers without creating
        // a scheduler. Production-equivalent tests use the fallible constructor.
        Self {
            to_write_tx,
    ...
    fn per_pane_for_registration(
        ...
        let coordinator = self.push_coordinator.as_ref();
        let tracked = self
            .per_pane
            .entry(pane_id)
            .and_modify(|tracked| {
                ...
                if !is_same {
                    tracked.live.store(false, Ordering::Release);
                    if let Some(coordinator) = coordinator {
                        coordinator.remove(pane_id);
                    }
    ...
    pub(crate) fn schedule_tracked_pane_push(&self, pane_id: PaneId) {
        if let Some(tracked) = self.per_pane.get(&pane_id) {
            let Some(_) = tracked.registration.as_ref() else {
                return;
            };
            loop {
                ...
                if current == PANE_PUSH_IDLE {
                    if let Some(coordinator) = &self.push_coordinator {
                        ...
                        tracked.push_state.store(PANE_PUSH_IDLE, Ordering::Release);
                    }
                }
                return;
            }
...
mod tests {
    ...
    fn reliable_pane_write_partial_ack_loss_replays_prefix_without_a_second_write() {
        ...
        assert_eq!(
            pane.writer_calls(),
            1,
            "ACK replay must not invoke Write again"
        );
        assert_eq!(pane.written_bytes(), b"abc");

        let mut wrong_registration = request;
        wrong_registration.input_serial = InputSerial::from_millis_since_epoch(121);
        wrong_registration.pane_registration =
            Some(ReliablePaneRegistrationIdentityV1::from_bytes([0x77; 16]));
        handler.process_one(DecodedPdu {
            serial: 5,
        ...
        assert!(matches!(
            take_response(&captured).pdu,
            Pdu::ReliablePaneWriteV1Response(ReliablePaneWriteV1Response {
                outcome: ReliablePaneWriteOutcomeV1::Rejected(ReliablePaneWriteRejectionV1::Input(
                    ReliableKeyEventRejectionV1::PaneRegistrationMismatch
                )),
                ..
            })
    ...
    fn reliable_pane_write_zero_error_and_panic_have_distinct_retry_authority() {
        ...
        assert_eq!(
            pane.writer_calls(),
            4,
            "ambiguous panic must quarantine replay"
        );
    }
    ...
    fn reliable_pane_write_cancel_before_effect_allows_exact_retry() {
        ...
        let pane_for_mux: Arc<dyn Pane> = pane.clone();
        mux.add_pane(&pane_for_mux)
            .expect("register cancellation pane");
        let registration = mux
            .capture_pane_registration(&pane_for_mux)
    ...
    fn reliable_pane_write_saturation_retries_exact_identity_without_effect() {
        ...
        let pane_for_mux: Arc<dyn Pane> = pane.clone();
        mux.add_pane(&pane_for_mux)
            .expect("register saturation pane");
        let registration = mux
            .capture_pane_registration(&pane_for_mux)
        ...
        assert!(matches!(
            take_response(&captured).pdu,
            Pdu::ReliablePaneWriteV1Response(ReliablePaneWriteV1Response {
                outcome: ReliablePaneWriteOutcomeV1::Retry(ReliablePaneWriteRetryV1::Input(
                    ReliableKeyEventRetryV1::SchedulerFull(_)
                )),
                ..
            })
        ));
    ...
    fn test_pane_entry_metadata_usage(entry: &mux::tab::PaneEntry) -> (usize, usize, usize) {
        let retained = entry.title.capacity()
            + entry
                .working_dir
                .as_ref()
        ...
        let encoded = test_snapshot_string_encoded_bytes(&entry.title)
            + entry.working_dir.as_ref().map_or(1, |url| {
                1 + test_snapshot_string_encoded_bytes(url.as_str())
            })
            + test_snapshot_string_encoded_bytes(&entry.workspace)
            + entry
                .tty_name
                .as_ref()
                .map_or(1, |tty| 1 + test_snapshot_string_encoded_bytes(tty));
        (4, retained, encoded)
    }

    fn test_pane_node_metadata_usage(node: &mux::tab::PaneNode) -> (usize, usize, usize) {
        match node {
            mux::tab::PaneNode::Empty => (0, 0, 0),
            mux::tab::PaneNode::Leaf(entry) => test_pane_entry_metadata_usage(entry),
            mux::tab::PaneNode::Split { left, right, .. } => {
                let left = test_pane_node_metadata_usage(left);
                let right = test_pane_node_metadata_usage(right);
                (left.0 + right.0, left.1 + right.1, left.2 + right.2)
            }
        }
    }

    fn test_snapshot_response_metadata_usage(
        snapshot: &ListPanesResponse,
    ) -> (usize, usize, usize) {
        let mut usage = (snapshot.window_titles.len(), 0usize, 0usize);
        for title in snapshot.window_titles.values() {
            usage.1 += title.capacity();
            usage.2 += test_snapshot_string_encoded_bytes(title);
        }
        usage.0 += snapshot.tab_titles.len();
        for title in &snapshot.tab_titles {
            usage.1 += title.capacity();
            usage.2 += test_snapshot_string_encoded_bytes(title);
        }
        for tree in &snapshot.tabs {
            let tree_usage = test_pane_node_metadata_usage(tree);
            usage.0 += tree_usage.0;
            usage.1 += tree_usage.1;
            usage.2 += tree_usage.2;
        }
        for floating in &snapshot.floating_panes {
            let pane_usage = test_pane_entry_metadata_usage(&floating.pane);
            usage.0 += pane_usage.0;
            usage.1 += pane_usage.1;
            usage.2 += pane_usage.2;
        }
        usage
    }
    ...
    fn pane_snapshot_census_q_scale_measurements_justify_attempt_ceiling() {
                ...
                .enumerate()
        {
            ...
            let response_usage = test_snapshot_response_metadata_usage(&snapshot);
            let attempt_metadata = metadata
                .attempt_stats()
                .total()
                .expect("q-scale metadata totals remain representable");
            assert_eq!(
                attempt_metadata.values,
                response_usage.0 + 1,
                "q{tab_count} owns exactly one additional temporary window-workspace admission"
            );
            assert_eq!(
                attempt_metadata.retained_bytes, response_usage.1,
                "q{tab_count} live producer retention must equal response-owned string capacity"
            );
            assert_eq!(
                attempt_metadata.encoded_bytes, response_usage.2,
    ...
    #[test]
    fn pane_snapshot_metadata_cancellation_releases_provisional_attempt_retention() {
        const TAB_COUNT: usize = 220;
        ...
        for tab_index in 0..TAB_COUNT {
            let tab = register_snapshot_tab(
                &mux,
                Arc::new(FakePane::new_with_id(40_000 + tab_index, None)),
            );
            mux.add_tab_to_window(&tab, window_id)
                .expect("attach cancellation-test tab");
        }
        ...
        assert!(
            ...
            "q{TAB_COUNT} must suspend at the first cooperative census yield"
        );
        ...
        assert_eq!(
            metadata.attempt_stats(),
            mux::tab::PaneSnapshotMetadataStats::default(),
            "dropping the pending producer must release every provisional response string"
        );
        assert!(
            metadata
                .request_stats()
                .total()
                .is_some_and(|usage| usage.retained_bytes > 0),
            "cancellation must preserve monotonic request work authority"
        );
        assert!(
            metadata.take_retry_released_bytes() > 0,
    ...
    fn pane_snapshot_census_contains_floating_callback_panic_before_output() {
        ...
        assert_eq!(
            ledger.attempt_stats().pane_callbacks,
            8,
            "the first float consumes six callbacks; the second admits its title before the injected dimensions panic"
        );

        metadata
            .release_attempt_to(Default::default())
            .expect("direct helper test releases its failed provisional metadata");
        let _ = metadata.take_retry_released_bytes();
        ledger.begin_attempt();
        metadata.begin_attempt();
        ...
        assert_eq!(
            metadata.attempt_stats(),
            mux::tab::PaneSnapshotMetadataStats::default(),
            "failed collection must release every provisional owned metadata value"
        );
        assert!(metadata.take_retry_released_bytes() > 0);
    }
    ...
    fn dimensions_request_retries_busy_metadata_without_publishing_hot_only_geometry() {
        ...
        let (sender, captured) = capturing_sender();
        let mut handler = SessionHandler::new_for_session(sender, SessionOwner::new(mux)).unwrap();
    ...
    #[test]
    fn session_render_owner_delivers_final_output_with_initial_render_capacity_full() {
        let _global = crate::GLOBAL_STATE_TEST_LOCK.lock().unwrap();
        let executor = SimpleExecutor::try_with_limits(
            MainThreadAdmissionLimits::new(2, 8 * 1024, 0, 0).unwrap(),
        )
        .unwrap();
        let mux = Arc::new(Mux::new(None));
        let pane = Arc::new(FakePane::new(None));
        ...
        drain_simple_executor(&executor);
        assert_eq!(executor.admission_snapshot().active_tasks, 1);
        let blocker = match try_reserve_main_thread_with_low_priority(
            MainThreadServiceClass::Render,
            MAIN_THREAD_RPC_BASE_ESTIMATED_BYTES,
        ) {
            MainThreadReservationOutcome::Reserved(reservation) => {
                reservation.spawn_local(std::future::pending::<()>())
            }
            other => panic!("expected remaining Render slot, got {other:?}"),
        };
        assert!(matches!(
            try_reserve_main_thread_with_low_priority(
                MainThreadServiceClass::Render,
                MAIN_THREAD_RPC_BASE_ESTIMATED_BYTES,
            ),
            MainThreadReservationOutcome::RetryableFull(_)
        ...
        let coordinator = handler.push_coordinator.as_ref().unwrap();
        assert_eq!(coordinator.queue.lock().unwrap().pending.len(), 1);
        assert_eq!(coordinator.queue.lock().unwrap().order.len(), 1);
        drain_simple_executor(&executor);
        assert!(
            captured.lock().unwrap().iter().any(|decoded| matches!(
                &decoded.pdu, Pdu::GetPaneRenderChangesResponse(response) if response.seqno == 12
            )),
            "the already-admitted owner must progress even before the blocker releases capacity"
        );
        assert!(coordinator.queue.lock().unwrap().pending.is_empty());
        drop(blocker);
        drain_simple_executor(&executor);
        assert_eq!(executor.admission_snapshot().active_tasks, 1);
        drop(handler);
    ...
    #[test]
    fn session_render_owner_skips_busy_pane_and_retires_queued_registration() {
        let _global = crate::GLOBAL_STATE_TEST_LOCK.lock().unwrap();
        for replace in [false, true] {
            let executor = SimpleExecutor::with_io_runtime().unwrap();
            let mux = Arc::new(Mux::new(None));
            let busy = Arc::new(FakePane::new_with_id(8_921, None));
            busy.coherent_surface.store(true, Ordering::Release);
            busy.line_layout_busy.store(true, Ordering::Release);
            busy.state.lock().unwrap().title = "retired-busy-source".to_string();
            let quiet: Arc<dyn Pane> = Arc::new(FakePane::new_with_id(8_922, None));
            let busy_dyn: Arc<dyn Pane> = busy.clone();
            mux.add_pane(&busy_dyn).unwrap();
            mux.add_pane(&quiet).unwrap();
            let original = mux.capture_pane_registration(&busy_dyn).unwrap();
            let (sender, captured) = capturing_sender();
            let mut handler = SessionHandler::new_for_mux(sender, Arc::clone(&mux)).unwrap();
            handler.schedule_pane_push(busy.pane_id());
            drain_simple_executor(&executor);
            assert!(captured.lock().unwrap().is_empty());
            assert_eq!(
                handler
            ...
            handler.schedule_pane_push(quiet.pane_id());
            drain_simple_executor(&executor);
            assert!(busy.line_layout_busy.load(Ordering::Acquire));
            assert!(captured.lock().unwrap().iter().any(|decoded| matches!(
                &decoded.pdu, Pdu::GetPaneRenderChangesResponse(response) if response.pane_id == quiet.pane_id()
            )), "quiet final output must not wait for the busy sibling's lock");
            let old_state = Arc::clone(&handler.per_pane[&busy.pane_id()].push_state);
            assert!(original.retire_if_current());
            if replace {
                let replacement = Arc::new(FakePane::new_with_id(busy.pane_id(), None));
                replacement.state.lock().unwrap().title = "replacement-source".to_string();
                let replacement_dyn: Arc<dyn Pane> = replacement;
                mux.add_pane(&replacement_dyn).unwrap();
                handler.schedule_pane_push(replacement_dyn.pane_id());
            } else {
                handler.remove_per_pane_if_same(&original);
            }
            assert_eq!(old_state.load(Ordering::Acquire), PANE_PUSH_IDLE);
            busy.line_layout_busy.store(false, Ordering::Release);
            drain_simple_executor(&executor);
            let responses = captured.lock().unwrap();
            assert!(!responses.iter().any(|decoded| matches!(
                &decoded.pdu, Pdu::GetPaneRenderChangesResponse(response) if response.title == "retired-busy-source"
            )));
            assert_eq!(responses.iter().filter(|decoded| matches!(
                &decoded.pdu, Pdu::GetPaneRenderChangesResponse(response) if response.title == "replacement-source"
            )).count(), usize::from(replace));
            drop(responses);
            drop(handler);
            drain_simple_executor(&executor);
        }
    ...
    #[test]
    fn session_render_owner_refuses_connection_without_capacity_and_clears_shutdown_work() {
        let _global = crate::GLOBAL_STATE_TEST_LOCK.lock().unwrap();
        let executor = SimpleExecutor::try_with_limits(
        ...
        let (sender, captured) = capturing_sender();
        let mut handler = SessionHandler::new_for_mux(sender.clone(), Arc::clone(&mux)).unwrap();
        assert!(SessionHandler::new_for_mux(sender, mux).is_err());
        handler.schedule_pane_push(pane.pane_id());
        let state = Arc::clone(&handler.per_pane[&pane.pane_id()].push_state);
        let coordinator = Arc::clone(handler.push_coordinator.as_ref().unwrap());
        // Drop before the owner's first poll, with a retained final update.
        drop(handler);
        assert_eq!(state.load(Ordering::Acquire), PANE_PUSH_IDLE);
        assert!(coordinator.queue.lock().unwrap().closed);
        assert!(coordinator.queue.lock().unwrap().pending.is_empty());
        drain_simple_executor(&executor);
        assert!(captured.lock().unwrap().is_empty());
        assert_eq!(executor.admission_snapshot().active_tasks, 0);
